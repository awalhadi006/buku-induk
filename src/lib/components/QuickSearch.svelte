		...
		<div class="modal-box w-[min(560px,calc(100vw-2rem))] max-h-[90vh] overflow-hidden">
			<div class="flex items-center gap-2 border-b border-base-300 px-4 py-3">
				...
				<input
					...
					oninput={onInput}
					type="text"
					class="input input-bordered flex-1 bg-transparent text-sm"
			...
			{#if show}
				...
				{:else}
					<ul class="dropdown-content menu p-2 max-h-[60vh] overflow-y-auto" role="listbox" aria-label="Hasil pencarian">
						{#each results as res, i (res.href + res.title)}
							<li role="option" aria-selected={i === selected}>
								<button
									class="btn btn-ghost btn-wide justify-start gap-3 rounded-xl px-3 py-2 text-left text-sm
										{i === selected ? 'btn-primary' : ''}"
