	...
	<section class="mt-6">
		...
		<div class="mt-4 rounded-lg border border-base-300 bg-base-100 p-4">
			<div class="flex items-center justify-between">
				...
				<form method="POST" action="?/toggleAdminTuCreateUsers" class="mt-4">
					<LoadingButton loading={submittingToggleAdmin} variant={settings['allow_admin_tu_create_users'] === 'true' ? 'success' : 'outline'} size="sm" class="mt-2" onclick={() => submittingToggleAdmin = true}>
		...
		{#if data.isSuperadmin || data.canCreateUsers}
			<form method="POST" action="?/createUser" class="mt-4 rounded-lg border border-base-300 bg-base-100 p-5">
				...
				<LoadingButton loading={submittingCreateUser} variant="primary" size="sm" class="mt-4" onclick={() => submittingCreateUser = true}>
							...
							<td class="text-right">
								{#if editingId === p.id}
									<form method="POST" action="?/updateProfile" class="flex justify-end gap-2">
										...
										<LoadingButton loading={submittingUpdateProfile} variant="primary" size="sm" class="mt-2" onclick={() => submittingUpdateProfile = true}>Simpan</LoadingButton>
									...
									<form method="POST" action="?/resetPassword" class="flex justify-end gap-2 mt-2">
										...
										<LoadingButton loading={submittingResetPassword} variant="warning" size="sm" ariaLabel="Reset Password" onclick={() => { if (confirm('Kirim link reset password ke email pengguna ini?')) submittingResetPassword = true; else return false; }}>Reset Password</LoadingButton>
									</form>
								{:else}
									<button
		...
		<div class="mt-4 grid gap-4 lg:grid-cols-2">
			{#each permissions as perm (perm.role)}
				<form method="POST" action="?/updatePermissions" class="rounded-lg border border-base-300 bg-base-100 p-5">
					...
					<LoadingButton loading={submittingUpdatePermissions} variant="primary" size="sm" class="mt-3" onclick={() => submittingUpdatePermissions = true}>Simpan izin</LoadingButton>
				</form>
			{/each}
		</div>
	</section>
					...
					{:else}
						<div class="flex flex-wrap items-center gap-3">
							...
							<form method="POST" action="?/deleteField" onsubmit={() => { if (confirm(`Hapus field "${f.label}"?`)) submittingDeleteField = true; else return false; }}>
								<input type="hidden" name="id" value={f.id} />
								<LoadingButton loading={submittingDeleteField} variant="ghost" size="sm" ariaLabel="Hapus field" type="submit">
...
{:else if tab === 'ta'}
	<section class="mt-6">
		...
		<form method="POST" action="?/updateSetting" class="mt-4 max-w-md rounded-lg border border-base-300 bg-base-100 p-5">
			...
			<LoadingButton loading={submittingUpdateSetting} variant="primary" size="sm" class="mt-4" onclick={() => submittingUpdateSetting = true}>Simpan</LoadingButton>
		</form>
		...
		<div class="mt-6 max-w-md">
						...
						<li class="flex items-center justify-between gap-3 px-4 py-2.5">
							...
							<div class="flex items-center gap-1">
								<form method="POST" action="?/toggleTahunAjaran">
									...
									<LoadingButton loading={submittingToggleTahunAjaran} variant="ghost" size="xs" class={ta.aktif ? 'text-success' : 'text-base-content/50'} onclick={() => submittingToggleTahunAjaran = true}>
								...
								<form method="POST" action="?/deleteTahunAjaran" onsubmit={() => { if (confirm('Hapus tahun ajaran ini?')) submittingDeleteTahunAjaran = true; else return false; }}>
									<input type="hidden" name="id" value={ta.id} />
									<LoadingButton loading={submittingDeleteTahunAjaran} variant="ghost" size="xs" ariaLabel="Hapus" type="submit" class="text-error">
										<IconTrash class="size-4" stroke-width={1.75} />
									</LoadingButton>
								</form>
							</div>
			...
			<form method="POST" action="?/createTahunAjaran" class="mt-3 flex gap-2">
				...
				<LoadingButton loading={submittingCreateTahunAjaran} variant="outline" size="sm" onclick={() => submittingCreateTahunAjaran = true}>
...
{:else if tab === 'nis' && data.isSuperadmin}
	<section class="mt-6 max-w-2xl">
		...
		<form method="POST" action="?/updateNisPattern" class="mt-4 rounded-lg border border-base-300 bg-base-100 p-5">
			...
			<LoadingButton loading={submittingUpdateNisPattern} variant="primary" size="sm" class="mt-4" onclick={() => submittingUpdateNisPattern = true}>
		...
		<form method="POST" action="?/bulkGenerateNis" class="mt-4 rounded-lg border border-base-300 bg-base-100 p-5">
			...
			<LoadingButton loading={submittingBulkGenerateNis} variant="outline" size="sm" class="mt-3" onclick={() => { if (confirm('Generate NIS untuk semua santri yang belum punya NIS?')) submittingBulkGenerateNis = true; else return false; }}>
		...
		<div class="mt-4 rounded-lg border p-5 {gdrive?.refresh_token ? 'border-success/40 bg-success/5' : 'border-base-300 bg-base-100'}">
			...
			{#if data.isSuperadmin}
				<form method="POST" action="?/updateGDriveFolder" class="mt-4">
					...
					<LoadingButton loading={submittingUpdateGDriveFolder} variant="primary" size="sm" class="mt-3" onclick={() => submittingUpdateGDriveFolder = true}>Simpan Folder</LoadingButton>
				</form>
			{:else}
...
{:else if tab === 'school'}
	<section class="mt-6 max-w-2xl">
		...
		<form
			...
			<LoadingButton loading={submittingUpdateSchoolIdentity} variant="primary" size="sm" class="mt-4" onclick={() => {
