export async function load({ locals }) {
	const [{ data: santriData }, { data: kamarData }, { data: kelasData }, { data: kabupatenData }] = await Promise.all([
		locals.supabase
			.from('santri')
			.select('id,nama_lengkap,nisn,nik,nis,nipd,jenis_kelamin,status_santri,status_keluarga,kamar_id,kelas_id,kabupaten,tempat_lahir,tanggal_lahir,wali_santri_id,kamar(nomor),kelas(tingkat,rombel)')
			.order('nama_lengkap'),
