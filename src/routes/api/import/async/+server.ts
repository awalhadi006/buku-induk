...
export const POST = async ({ request, locals }) => {
	...
	if (!(file instanceof File)) {
		return json({ error: 'Pilih file Excel dulu.' }, { status: 400 });
	}
	if (!/\.(xlsx|xls)$/i.test(file.name)) {
		return json({ error: 'Hanya file .xlsx atau .xls yang didukung.' }, { status: 400 });
	}
	...
	if (rows.length === 0) {
		...
		if (!ws) {
			return json({ error: 'Sheet tidak ditemukan.' }, { status: 400 });
		}
